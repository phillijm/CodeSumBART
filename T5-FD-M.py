""" T5 (Seq-2-Seq) Encoder/Decoder Transformer
    We tuned this model on the Funcom Dataset, using METEOR evaluation.
...
baseName = "t5-small"
metricName = "meteor"
outputDir = "./Exp1-FuncomDataset-Results-M"
...
def computeMetrics(predictions, labels):
    ...
    meteor = metric.compute(predictions=predictions,
...
# ------------------------------- Save Model ----------------------------------
    if result["METEOR"] > previousValidationResult:  # Don't kill best model.
        previousValidationResult = result["METEOR"]
