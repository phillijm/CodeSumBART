""" T5 (Seq-2-Seq) Encoder/Decoder Transformer
    We tuned this model on the Funcom Dataset, using ROUGE-L evaluation.
...
baseName = "t5-small"
metricName = "rouge"
outputDir = "./Exp1-FuncomDataset-Results-RL"
...
def computeMetrics(predictions, labels):
    ...
    rl = metric.compute(predictions=predictions,
                        references=refs,
                        rouge_types=["rougeL"],
...
# ------------------------------- Save Model ----------------------------------
    if result["ROUGE-L"] > previousValidationResult:  # Don't kill best model.
        previousValidationResult = result["ROUGE-L"]
