""" T5 (Seq-2-Seq) Encoder/Decoder Transformer
    We tuned this model on the Funcom Dataset, using FrugalScore evaluation.
...
baseName = "t5-small"
metricName = "frugalscore"
outputDir = "./Exp1-FuncomDataset-Results-FS"
...
def computeMetrics(predictions, labels):
    ...
    fs = metric.compute(predictions=predictions,
                        ...
                        batch_size=48,
                        device='gpu')["scores"]
    meanFs = sum(fs) / len(fs)
    return {"FrugalScore": meanFs}
...
# ------------------------------- Save Model ----------------------------------
    if result["FrugalScore"] > previousValidationResult:
        # Don't kill best model.
        previousValidationResult = result["FrugalScore"]
