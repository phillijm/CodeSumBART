...
class AdamWBARTMETEORModel(pl.LightningModule):
    ...
    def __init__(self, model: str, *args: tuple, **kwargs: dict) -> None:
        ...
        """
        super(AdamWBARTMETEORModel, self).__init__()
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> float:
        ...
        shutil.rmtree(f"./{now}")  # Delete METEOR's temprary files.
        self.log("val_meteor", mt, on_step=False, on_epoch=True, prog_bar=True)
        return mt
...
if __name__ == '__main__':
    ...
    checkpointCallback = ModelCheckpoint(monitor="val_meteor",
                                         save_top_k=3,
                                         mode="max",
    ...
    model = AdamWBARTMETEORModel(model=base,
                                 use_stemmer=True,
    ...
    trainer = pl.Trainer(accelerator="gpu",
                         ...
                         callbacks=[checkpointCallback,
                                    EarlyStopping(monitor="val_meteor",
                                                  mode="max",
                                                  patience=5)])
