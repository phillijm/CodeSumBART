...
class AdamWBARTFrugalScoreModel(pl.LightningModule):
    ...
    def __init__(self, model: str, *args: tuple, **kwargs: dict) -> None:
        ...
        """
        super(AdamWBARTFrugalScoreModel, self).__init__()
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> float:
        ...
        fs = metric.compute(predictions=decodedPreds,
                            references=decodedLabels,
                            batch_size=48,
                            device='gpu')["scores"]
        frugal = sum(fs) / len(fs)
        self.log("val_fs", frugal, on_step=False, on_epoch=True, prog_bar=True)
        return frugal
...
if __name__ == '__main__':
    ...
    checkpointCallback = ModelCheckpoint(monitor="val_fs",
                                         save_top_k=3,
                                         mode="max",
    ...
    model = AdamWBARTFrugalScoreModel(model=base,
                                      use_stemmer=True,
