...
class AdamWBARTRougeModel(pl.LightningModule):
    ...
    def __init__(self, model: str, *args: tuple, **kwargs: dict) -> None:
        ...
        """
        super(AdamWBARTRougeModel, self).__init__()
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> float:
        ...
        rouge = metric.compute(predictions=decodedPreds,
                               references=decodedLabels,
                               rouge_types=["rougeL"],
                               use_stemmer=True)["rougeL"]
        self.log("val_r", rouge, on_step=False, on_epoch=True, prog_bar=True)
        return rouge
...
if __name__ == '__main__':
    ...
    checkpointCallback = ModelCheckpoint(monitor="val_r",
                                         save_top_k=3,
                                         mode="max",
    ...
    model = AdamWBARTRougeModel(model=base,
                                use_stemmer=True,
