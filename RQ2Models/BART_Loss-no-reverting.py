...
class AdamWBARTModel(pl.LightningModule):
    ...
    def __init__(self, model: str, *args: tuple, **kwargs: dict) -> None:
        ...
        """
        super(AdamWBARTModel, self).__init__()
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> float:
        ...
        attention_mask = batch["attention_mask"]
        labels = batch["labels"]
        val_loss, _ = self(input_ids, attention_mask, labels)
        self.log("val_loss",
                 val_loss,
                 on_step=False,
                 on_epoch=True,
...
if __name__ == '__main__':
    ...
    checkpointCallback = ModelCheckpoint(monitor="val_loss",
                                         save_top_k=3,
                                         mode="max",
    ...
    model = AdamWBARTModel(model=base,
                           use_stemmer=True,
